        ...
        try {
            if (type === 'task') {
                ...
                tasksToUpdate.forEach(t => {
                    const taskRef = doc(db, `/artifacts/${appId}/public/data/tasks`, t.id);
                    batch.update(taskRef, { dependencies: t.dependencies.filter(depId => depId !== item.id) });
                });
                // Elimina task
                const taskRef = doc(db, `/artifacts/${appId}/public/data/tasks`, item.id);
                batch.delete(taskRef);
                await batch.commit();
                setNotification({message: "Attività eliminata.", type: "success"});
            } else if (type === 'project') {
                const batch = writeBatch(db);
                // Elimina tutte le attività del progetto
                ...
                // Elimina progetto
                const projectRef = doc(db, `/artifacts/${appId}/public/data/projects`, item.id);
