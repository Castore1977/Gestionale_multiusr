...
import { ArrowRight, Plus, Users, Trash2, Edit, LayoutDashboard, BarChart3, X, AlertTriangle, FileDown, FileUp, CheckCircle } from 'lucide-react';
...
const ActivityReportView = ({ projects, tasks, resources, onExportPDF }) => {
    const reportData = useMemo(() => { const data = projects.map(project => { let projectTotalHours = 0; let projectWorkedHours = 0; let projectTotalCost = 0; let projectSpentCost = 0; const projectTasks = tasks.filter(t => t.projectId === project.id).sort((a,b) => new Date(a.endDate) - new Date(b.endDate)).map(task => { const durationInDays = calculateDaysDifference(task.startDate, task.endDate) + 1; const totalTaskHours = durationInDays * (task.dailyHours || 8); const workedHours = totalTaskHours * ((task.completionPercentage || 0) / 100); const assigned = task.assignedResources?.map(resId => resources.find(r => r.id === resId)).filter(Boolean) || []; const combinedHourlyRate = assigned.reduce((sum, res) => sum + (res.hourlyCost || 0), 0); const totalEstimatedCost = totalTaskHours * combinedHourlyRate; const spentCost = totalEstimatedCost * ((task.completionPercentage || 0) / 100); projectTotalHours += totalTaskHours; projectWorkedHours += workedHours; projectTotalCost += totalEstimatedCost; projectSpentCost += spentCost; return { ...task, assigned, totalEstimatedCost, spentCost, totalTaskHours, workedHours }; }); return {...project, tasks: projectTasks, projectTotalHours, projectWorkedHours, projectTotalCost, projectSpentCost }; }); return data; }, [projects, tasks, resources]);
    return (
...
const CostReportView = ({ projects, tasks, resources, onExportPDF }) => {
    const reportData = useMemo(() => { const data = projects.map(project => { let projectTotalCost = 0; let projectSpentCost = 0; const projectTasks = tasks.filter(t => t.projectId === project.id).map(task => { const durationInDays = calculateDaysDifference(task.startDate, task.endDate) + 1; const totalTaskHours = durationInDays * (task.dailyHours || 8); const assigned = task.assignedResources?.map(resId => resources.find(r => r.id === resId)).filter(Boolean) || []; const combinedHourlyRate = assigned.reduce((sum, res) => sum + (res.hourlyCost || 0), 0); const totalEstimatedCost = totalTaskHours * combinedHourlyRate; const spentCost = totalEstimatedCost * ((task.completionPercentage || 0) / 100); projectTotalCost += totalEstimatedCost; projectSpentCost += spentCost; return { ...task, totalEstimatedCost, spentCost, assigned }; }); return { ...project, tasks: projectTasks, projectTotalCost, projectSpentCost }; }); const grandTotalCost = data.reduce((sum, p) => sum + p.projectTotalCost, 0); const grandSpentCost = data.reduce((sum, p) => sum + p.projectSpentCost, 0); return { projects: data, grandTotalCost, grandSpentCost }; }, [projects, tasks, resources]);
    return (
        <div className="p-4 md:p-6 lg:p-8">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50"><tr><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Attività</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Risorse</th><th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Costo Stimato</th><th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Costo Sostenuto</th></tr></thead>
                     <tbody className="bg-white divide-y divide-gray-200">
                        {reportData.projects.flatMap(project => [
                            <tr key={project.id} className="bg-gray-100"><td colSpan="4" className="px-6 py-3 text-sm font-bold text-gray-900" style={{backgroundColor: project.color, color: 'white'}}>{project.name}</td></tr>,
                    ...
                    <tfoot className="bg-gray-200"><tr><td colSpan="2" className="px-6 py-4 text-base font-bold text-right">TOTALE GENERALE</td><td className="px-6 py-4 text-right text-base font-bold">{formatCurrency(reportData.grandTotalCost)}</td><td className="px-6 py-4 text-right text-base font-bold">{formatCurrency(reportData.grandSpentCost)}</td></tr></tfoot>
                </table>
            </div>
        </div>
    );
...
const MainDashboard = ({ projects, tasks, resources, db, userId }) => {
    ...
    const tasksByProject = useMemo(() => { const grouped = {}; projects.forEach(p => { grouped[p.id] = { ...p, tasks: processedTasks.filter(t => t.projectId === p.id).sort((a,b) => (a.order || 0) - (b.order || 0)) } }); return projects.map(p => grouped[p.id]); }, [processedTasks, projects]);
    const flatTasksSorted = useMemo(() => tasksByProject.flatMap(p => p.tasks), [tasksByProject]);
    ...
    const handleDeleteTask = async (taskId) => { if (!window.confirm('Sei sicuro di voler eliminare questa attività?')) return; try { await deleteDoc(doc(db, `/artifacts/${appId}/public/data/tasks`, taskId)); const batch = writeBatch(db); tasks.forEach(t => { if (t.dependencies && t.dependencies.includes(taskId)) { const taskRef = doc(db, `/artifacts/${appId}/public/data/tasks`, t.id); batch.update(taskRef, { dependencies: t.dependencies.filter(depId => depId !== taskId) }); } }); await batch.commit(); } catch (error) { console.error("Errore eliminazione task:", error); } };
    ...
    return (
        <div className="h-screen w-screen bg-gray-100 flex flex-col">
            ...
            <main className="flex-grow overflow-auto">
                {view === 'gantt' ? ( <div ref={ganttContentRef} className="flex h-full min-w-max"> <div className="w-96 border-r bg-gray-50 flex-shrink-0"> <div className="h-12 flex items-center justify-between px-4 border-b bg-gray-100 font-semibold text-gray-700 sticky top-0 z-20">Attività per Progetto <button onClick={() => exportToPDF('gantt')} className="text-blue-600 hover:text-blue-800 p-1"><FileDown size={18}/></button></div> <div className="divide-y divide-gray-200"> {tasksByProject.map(project => ( <div key={project.id}> <div onClick={() => setSelectedProjectId(project.id)} className={`h-12 flex items-center justify-between px-4 sticky top-12 z-10 cursor-pointer transition-all ${selectedProjectId === project.id ? 'bg-blue-200 border-l-4 border-blue-600' : 'bg-gray-200'}`}> <div className="flex items-center gap-3"> <span className="w-4 h-4 rounded-full" style={{backgroundColor: project.color}}></span> <h3 className="font-bold text-gray-800">{project.name}</h3> </div> <button onClick={(e) => {e.stopPropagation(); handleEditProject(project)}} className="text-gray-500 hover:text-blue-600"><Edit size={16}/></button> </div> {project.tasks.map(task => ( <div key={task.id} className="p-2 pl-9 flex items-center group" style={{height: `${ROW_HEIGHT}px`}} onDoubleClick={() => handleEditTask(task)}> <div className="flex-grow"> <p className="font-medium text-gray-800">{task.name}</p> <div className="flex flex-wrap gap-1 mt-1"> {task.assignedResources?.map(resId => { const r = getResourceById(resId); return r ? <span key={resId} className="text-xs bg-gray-200 text-gray-800 px-1.5 py-0.5 rounded-full">{r.name}</span> : null; })} </div> </div> <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity"> <button onClick={(e) => {e.stopPropagation(); handleEditTask(task)}} className="p-1 text-gray-500 hover:text-blue-600"><Edit size={16} /></button> <button onClick={(e) => {e.stopPropagation(); handleDeleteTask(task.id)}} className="p-1 text-gray-500 hover:text-red-600"> <Trash2 size={16}/> </button> </div> </div> ))} {project.tasks.length === 0 && <div className="pl-9 text-xs text-gray-500 italic h-10 flex items-center">Nessuna attività.</div>} </div> ))} </div> </div> <div className="flex-grow h-full overflow-x-scroll" onDragOver={(e)=>e.preventDefault()} onDrop={handleGanttDrop}> <div className="relative" style={{width: `${dateHeaders.length * DAY_WIDTH}px`, height: `${flatTasksSorted.length * ROW_HEIGHT + tasksByProject.length * PROJECT_HEADER_HEIGHT + 48}px`}}> <div className="sticky top-0 z-30 flex bg-gray-100 h-12 border-b"> {dateHeaders.map((date, index) => ( <div key={index} className="w-10 text-center border-r flex-shrink-0"> <div className={`text-xs ${date.getDay() === 0 || date.getDay() === 6 ? 'text-red-500' : 'text-gray-500'}`}>{['D', 'L', 'M', 'M', 'G', 'V', 'S'][date.getDay()]}</div> <div className="text-sm font-semibold text-gray-800">{date.getDate()}</div> </div> ))} </div> <div className="absolute top-12 left-0 h-full w-full pointer-events-none z-0"> {dateHeaders.map((date, index) => (<div key={index} className={`absolute top-0 bottom-0 w-10 border-r ${(date.getDay() === 0 || date.getDay() === 6) ? 'bg-gray-100/50' : ''}`} style={{ left: `${index * DAY_WIDTH}px` }}></div>))}</div> <div className="absolute top-12 left-0 w-full h-full z-10 pointer-events-none"> <svg width="100%" height="100%"> <defs> <marker id="arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#4a5568" /></marker> </defs> {flatTasksSorted.map((task) => task.dependencies?.map(depId => { const predecessor = flatTasksSorted.find(t => t.id === depId); if (!predecessor) return null; let fromFlatIndex = flatTasksSorted.findIndex(t => t.id === predecessor.id); let toFlatIndex = flatTasksSorted.findIndex(t => t.id === task.id); let fromProjectCount = tasksByProject.filter((p, i) => tasksByProject.slice(0,i).reduce((acc,cur)=>acc+cur.tasks.length,0) <= fromFlatIndex).length; let toProjectCount = tasksByProject.filter((p, i) => tasksByProject.slice(0,i).reduce((acc,cur)=>acc+cur.tasks.length,0) <= toFlatIndex).length; const fromY = fromFlatIndex * ROW_HEIGHT + fromProjectCount * PROJECT_HEADER_HEIGHT + ROW_HEIGHT/2; const toY = toFlatIndex * ROW_HEIGHT + toProjectCount * PROJECT_HEADER_HEIGHT + ROW_HEIGHT/2; const fromX = (calculateDaysDifference(dateHeaders[0], predecessor.endDate) + 1) * DAY_WIDTH; const toX = calculateDaysDifference(dateHeaders[0], task.startDate) * DAY_WIDTH; const path = `M ${fromX} ${fromY} C ${fromX + DAY_WIDTH/2} ${fromY}, ${toX - DAY_WIDTH/2} ${toY}, ${toX-8} ${toY}`; return <path key={`${depId}-${task.id}`} d={path} stroke="#4a5568" strokeWidth="2" fill="none" markerEnd="url(#arrow)" /> }))} </svg> </div> <div className="absolute top-12 left-0 w-full h-full z-20"> {tasksByProject.reduce((acc, project) => { const projectTop = acc.totalHeight; const projectHeader = <div key={`header-${project.id}`} className="absolute w-full h-12" style={{top: `${projectTop}px`}}></div>; const taskElements = project.tasks.map((task, taskIndex) => { const left = calculateDaysDifference(dateHeaders[0], task.startDate) * DAY_WIDTH; const width = task.duration * DAY_WIDTH; const top = projectTop + PROJECT_HEADER_HEIGHT + (taskIndex * ROW_HEIGHT); const bgColor = task.taskColor || project.color || '#3b82f6'; return ( <div key={task.id} className="absolute flex items-center" style={{ top: `${top}px`, height: `${ROW_HEIGHT}px`, left: `${left}px`, width: `${width}px` }} onDoubleClick={()=>handleEditTask(task)}> <div draggable onDragStart={(e) => handleDragStart(e, task, 'move')} className="h-8 rounded-md shadow-sm flex items-center w-full group relative" style={{ backgroundColor: bgColor }}> <div className="absolute top-0 left-0 h-full rounded-l-md" style={{width: `${task.completionPercentage || 0}%`, backgroundColor: 'rgba(0,0,0,0.2)'}}></div> <div className="absolute px-2 text-white text-sm truncate font-medium z-10">{task.name}</div> <div draggable onDragStart={(e) => { e.stopPropagation(); handleDragStart(e, task, 'resize-start'); }} className="absolute left-0 top-0 w-2 h-full cursor-ew-resize z-20" /> <div draggable onDragStart={(e) => { e.stopPropagation(); handleDragStart(e, task, 'resize-end'); }} className="absolute right-0 top-0 w-2 h-full cursor-ew-resize z-20" /> </div> </div> ); }); acc.elements.push(projectHeader, ...taskElements); acc.totalHeight += PROJECT_HEADER_HEIGHT + (project.tasks.length * ROW_HEIGHT); return acc; }, {elements:[], totalHeight:0}).elements} </div> </div> </div> </div>
                ) : view === 'costReport' ? ( <CostReportView projects={projects} tasks={tasks} resources={resources} onExportPDF={() => exportToPDF('cost')} />
                ) : ( <ActivityReportView projects={projects} tasks={tasks} resources={resources} onExportPDF={() => exportToPDF('activity')} /> )
                }
            </main>
