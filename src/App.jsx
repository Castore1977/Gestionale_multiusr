...
const ActivityReportView = ({ projects, tasks, resources, onExportPDF }) => {
    const reportData = useMemo(() => { const today = new Date(); today.setHours(0, 0, 0, 0); const threeDaysFromNow = new Date(); threeDaysFromNow.setDate(today.getDate() + 3); const allEnrichedTasks = tasks.map(task => { const project = projects.find(p => p.id === task.projectId); const durationInDays = calculateDaysDifference(task.startDate, task.endDate) + 1; const totalTaskHours = durationInDays * (task.dailyHours || 8); const assigned = task.assignedResources?.map(resId => resources.find(r => r.id === resId)).filter(Boolean) || []; const combinedHourlyRate = assigned.reduce((sum, res) => sum + (res.hourlyCost || 0), 0); const totalEstimatedCost = totalTaskHours * combinedHourlyRate; const spentCost = totalEstimatedCost * ((task.completionPercentage || 0) / 100); return { ...task, projectName: project?.name || 'N/D', projectColor: project?.color || '#cccccc', assigned, totalEstimatedCost, spentCost, totalTaskHours }; }); const dueTodayTasks = allEnrichedTasks.filter(task => { const d = new Date(task.endDate); d.setHours(0,0,0,0); return d.getTime() === today.getTime(); }).sort((a,b) => a.projectName.localeCompare(b.projectName)); const dueInThreeDaysTasks = allEnrichedTasks.filter(task => { const d = new Date(task.endDate); d.setHours(0,0,0,0); return d > today && d <= threeDaysFromNow; }).sort((a,b) => new Date(a.endDate) - new Date(b.endDate)); const otherTasks = allEnrichedTasks.filter(task => { const d = new Date(task.endDate); d.setHours(0,0,0,0); return d.getTime() !== today.getTime() && (d < today || d > threeDaysFromNow) }).sort((a,b) => new Date(a.endDate) - new Date(b.endDate)); return { dueTodayTasks, dueInThreeDaysTasks, otherTasks }; }, [projects, tasks, resources]);
